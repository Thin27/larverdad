...
$per_page = $apollo13framework_a13->get_option( 'albums_list_items_per_page' );
$args = array(
...
$title = '';
if ( ! $ajax_call ) {
	$title = get_the_title( $apollo13framework_a13->get_option( 'albums_list_page' ) );
	...
	if ( $genre_template === true ) {
		...
		if ( ! empty( $term_slug ) ) {
			...
			$title    = sprintf( '%1$s : %2$s', $title, $term_obj->name );
		}
	}
}
