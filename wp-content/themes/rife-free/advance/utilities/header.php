...
if ( ! function_exists( 'apollo13framework_header_logo' ) ) {
	...
	function apollo13framework_header_logo() {
		...
		//check for WordPres feature logo if theme logo is not present
		...
		if($icons > 0 || $allow_mobile_menu || $is_button){
			$html = '
				<div id="header-tools" class="' . esc_attr( $classes ) . '">' .
			        ( $basket_sidebar ? '<div id="basket-menu-switch" class="'.esc_attr($basket_sidebar_icon).' tool" title="' . esc_attr__( 'Shop sidebar', 'rife-free' ) . '"><span id="basket-items-count" class="zero">' . esc_html( $woocommerce->cart->cart_contents_count ) . '</span></div>' : '' ) .
			        ( $header_search ? '<div id="search-button" class="'.esc_attr($header_search_icon).' tool" title="' . esc_attr__( 'Search', 'rife-free' ) . '"></div>' : '' ) .
			        ( $hidden_sidebar ? '<div id="side-menu-switch" class="'.esc_attr($hidden_sidebar_icon).' tool" title="' . esc_attr__( 'Hidden sidebar', 'rife-free' ) . '">'.($hs_type === 'animated'? '<span>'.esc_html__( 'Hidden sidebar', 'rife-free' ).'</span>' : '').'</div>' : '' ) .
			        ( $menu_overlay ? '<div id="menu-overlay-switch" class="'.esc_attr($menu_overlay_icon).' tool" title="' . esc_attr__( 'Main menu', 'rife-free' ) . '">'.($mo_type === 'animated'? '<span>'.esc_html__( 'Main menu', 'rife-free' ).'</span>' : '').'</div>' : '' ) .
			        ( $allow_mobile_menu ? '<div id="mobile-menu-opener" class="'.esc_attr($mobile_menu_icon).' tool" title="' . esc_attr__( 'Main menu', 'rife-free' ) . '">'.($mm_type === 'animated'? '<span>'.esc_html__( 'Main menu', 'rife-free' ).'</span>' : '').'</div>' : '' ) .
